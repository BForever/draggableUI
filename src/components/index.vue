<template>
  <div>
    <md-drawer :md-active.sync="showNavigation" md-swipeable md-persistent="full">
      ...
      <draggable :clone="cloneData" :list="form_list" :options="dragOptions1">
        <transition-group class="form-list-group" type="transition" :name="'flip-list'" tag="div">
          <renders v-for="(element,index) in form_list" :key="index" :ele="element.ele"
                   :obj="element.obj || {}" :config-icon="true" style="margin-top: 20px"></renders>
        </transition-group>
      </draggable>
    </md-drawer>

    <md-card>
<!--      <Alert style="margin: 15px 15px 0;" type="warning" show-icon>未绑定数据源控件无效</Alert>-->
      <md-card-header>
        <div class="md-title" @click="showNavigation=true">{{this.objFormalName}}</div>
      </md-card-header>
      ...
      <Form class="form_content" :label-width="80" :model="modalFormData" ref="modalFormData">
        ...
        <FormItem label="数据字典：" v-if="showModal">
          <Select v-model="modalFormData.dict" filterable @on-change="handleDataDictChange">
            ...
            <Option v-for="item in dataDictList" :value="JSON.stringify(item)" :key="item.id">{{ item.label }}
            </Option>
            <!--              <Option v-for="item in dataDictList" :value="JSON.stringify({-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 克隆表单提交事件
      handleSubmit() {
        // localStorage.setItem('template_form1', JSON.stringify(this.sortable_item1.filter(v => {
        //   return !!v.obj.name
        // })));
        let form = {
        ...
        // localStorage.setItem('template_form1', JSON.stringify(this.sortable_item1));
        // localStorage.setItem('template_form2', JSON.stringify(this.sortable_item2));
        this.$router.push('/render?obj='+this.objName);
      },
      // 清空克隆表单
      handleReset() {
        this.sortable_item1 = [{
          "obj" : {
            "visibility" : true,
            "modalTitle" : "Single State Device",
    ...
    created() {
      ...
      // this.sortable_item1 = JSON.parse(localStorage.getItem('template_form1') || '[]');
      // this.sortable_item2 = JSON.parse(localStorage.getItem('template_form2') || '[]');
    }
  };
</script>
<style>
